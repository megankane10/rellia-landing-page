  ...
  return (
    <div className={cn("pointer-events-none absolute inset-0 h-full w-full", className)}>
      ...
      >
        <g opacity="0.06">
          {pathData.map((d, i) => (
            <path key={`static-${i}`} d={d} stroke="#9DD6D0" strokeWidth="0.5" />
          ))}
        </g>
        ...
        <defs>
          {pathData.map((_, i) => (
            <linearGradient key={`gradient-${i}`} id={`gradient-${i}`} x1="0%" y1="0%" x2="100%" y2="100%">
              <stop offset="0%" stopColor="#9DD6D0" stopOpacity="0" />
              <stop offset="22%" stopColor="#9DD6D0" stopOpacity="1" />
              <stop offset="52%" stopColor="#9DD6D0" stopOpacity="1" />
              <stop offset="80%" stopColor="#9DD6D0" stopOpacity="1" />
              <stop offset="100%" stopColor="#9DD6D0" stopOpacity="0" />
            </linearGradient>
          ))}
        </defs>
      </svg>
