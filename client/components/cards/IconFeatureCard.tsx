...
export type IconFeatureCardProps = {
   ...
   * `interactive` — homepage cards: teal hover, lift, mint icon accents.
   */
  ...
  if (variant === "interactive") {
    return (
      ...
      >
        <div
          className={cn(
            "flex h-14 w-14 shrink-0 items-center justify-center rounded-xl bg-rellia-mint/20 transition-all duration-300",
            "group-hover:bg-white/10",
          )}
        >
          <Icon
            className="h-7 w-7 text-rellia-teal transition-colors duration-300 group-hover:text-rellia-mint"
            strokeWidth={1.75}
          />
        </div>
        <h3
          className={cn(
            "font-host-grotesk text-2xl font-bold leading-tight tracking-tight text-black md:text-[28px]",
        ...
        <p
          className={cn(
            "font-urbanist text-base font-medium leading-relaxed tracking-tight text-black/70 md:text-[18px]",
            "transition-colors duration-300 group-hover:text-white/85",
          )}
        >
          {description}
        </p>
