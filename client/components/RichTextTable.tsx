...
const MOBILE_COL_MIN_WIDTH = "min-w-[9.25rem] w-[9.25rem] max-md:min-w-[9.25rem] max-md:w-[9.25rem]"

const firstColBgClass = (stickyFirstColumn: boolean) =>
  stickyFirstColumn ? "max-md:bg-rellia-cream md:bg-rellia-cream/35" : "bg-rellia-cream/35"

const firstColStickyClass = (stickyFirstColumn: boolean) =>
  stickyFirstColumn
    ? "max-md:sticky max-md:left-0 max-md:z-[2] max-md:shadow-[4px_0_10px_-4px_rgba(0,0,0,0.14)]"
    : ""
...
const headerCellClass = (
  ...
  cn(
    ...
    MOBILE_COL_MIN_WIDTH,
    "md:min-w-0 md:w-auto",
    colIndex === 0 &&
      cn(firstColBgClass(stickyFirstColumn), firstColStickyClass(stickyFirstColumn), FIRST_COL_TEXT_CLASS),
...
export const RichTextTable = ({
  ...
  return (
    <figure className={cn("my-10 md:my-12 [&:first-child]:mt-0", className)}>
      ...
      >
        <table className="w-full min-w-[37rem] border-collapse text-left md:min-w-0 md:table-auto">
          {headerCells ? (
            <thead>
