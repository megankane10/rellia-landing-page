...
export default function Payment() {
  ...
  // Stripe Embedded Checkout — requires VITE_STRIPE_PUBLISHABLE_KEY + price IDs in .env.local
  const stripePublishableKey = (import.meta.env.VITE_STRIPE_PUBLISHABLE_KEY as string | undefined)?.trim()
  const monthlyPriceId = (import.meta.env.VITE_STRIPE_MONTHLY_PRICE_ID as string | undefined)?.trim()
  const annualPriceId = (import.meta.env.VITE_STRIPE_ANNUAL_PRICE_ID as string | undefined)?.trim()
  const selectedPriceId = selectedPlan === "annual" ? annualPriceId : monthlyPriceId
  ...
  const canEmbedCheckout = Boolean(stripePromise && selectedPriceId)
  ...
  const fetchClientSecret = useCallback(async (): Promise<string> => {
    ...
    const data = (await r.json()) as { clientSecret?: string; error?: string }
        ...
        <section className="relative w-full border-t border-black/5 pt-24 md:pt-10">
          ...
          <AnimatePresence mode="wait">
            {!showForm ? (
              ...
              >
                {/* Left: Order Summary */}
                <div className="flex-1 flex flex-col justify-start pt-16 pb-14 md:pt-28 md:pb-20 pr-6 md:pr-16">
                  <div className="relative z-10 w-full max-w-[500px]">
                    <button
                      type="button"
                      onClick={() => setShowForm(false)}
                      className="inline-flex items-center gap-2 font-host-grotesk text-sm font-bold text-rellia-mint hover:text-white transition-colors mb-12"
                    >
                      <ArrowLeft className="h-4 w-4" aria-hidden />Back to details
                    </button>
                    <p className="font-host-grotesk text-xs font-bold uppercase tracking-widest text-rellia-mint mb-4">
                      Order summary
                    </p>
                    <div className="mb-8">
                      <div className="flex items-baseline gap-2 mb-2">
                        <span className="font-host-grotesk text-4xl font-bold text-white tracking-tight">
                          {selectedPlan === "annual" ? p.pricingAnnualAmount : p.pricingMonthlyAmount}
                        </span>
                        <span className="font-urbanist text-white/50 text-base">
                          /{selectedPlan === "annual" ? "yr" : "mo"}
                        </span>
                      </div>
                      <p className="font-urbanist text-white/70 text-base">
                        Rellia Network · {selectedPlan === "annual" ? "Annual" : "Monthly"} membership
                      </p>
                    </div>
                    <div className="w-full h-px bg-white/10 mb-8" />
                    <div className="flex flex-col gap-4 mb-10">
                      {benefitsGrid.slice(0, 3).map((benefit, index) => (
                        <div key={index} className="flex items-start gap-3">
                          <CheckCircle2 className="mt-0.5 h-5 w-5 text-rellia-mint flex-shrink-0" />
                          <p className="font-urbanist text-white/80 text-sm leading-relaxed">{benefit}</p>
                        </div>
                      ))}
                    </div>
                    <div className="flex items-center gap-2">
                      <ShieldCheck className="h-4 w-4 text-rellia-mint/60" />
                      <span className="font-host-grotesk text-xs text-white/40 uppercase tracking-wider">
                        Secured by Stripe
                      </span>
                    </div>
                  </div>
                ...
                <div className="flex-1 flex flex-col justify-start pt-16 pb-14 md:pt-28 md:pb-20 md:pl-16">
                  <div className="w-full">
                    <div className="flex items-center justify-between mb-6">
                      <h2 className="font-host-grotesk text-2xl font-bold text-black">Complete payment</h2>
                      {currentHref && (
                        <a
                          href={currentHref}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-1.5 font-host-grotesk text-xs font-bold text-rellia-teal hover:underline hover:underline-offset-4"
                          aria-label="Open Stripe checkout in a new tab"
                        >
                          Open in new tab
                    ...
                    {canEmbedCheckout ? (
                      ...
                      <div className="flex flex-col items-center justify-center py-20 px-6 text-center gap-6 bg-white rounded-2xl border border-black/5">
                        <ShieldCheck className="h-12 w-12 text-rellia-teal" aria-hidden />
                        <p className="font-urbanist text-base text-black/60 max-w-sm">
                          Complete your payment securely on Stripe.
                        </p>
                        <RelliaAction asChild variant="mintTealFill" size="comfortable">
                      ...
                      <div className="flex flex-col items-center justify-center py-40 px-6 text-center bg-white rounded-2xl border border-black/5">
