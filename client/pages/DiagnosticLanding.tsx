...
const DiagnosticHeroBadges = ({ label }: { label: string }) => (
  <div className="flex flex-wrap items-center gap-3">
    <PillTag label={label} className={PILL_ON_IMAGE_BLUR_CLASS} />
  </div>
)
...
function DiagnosticFallback({ content }: { content: DiagnosticLandingPageContent }) {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-white font-host-grotesk text-black selection:bg-rellia-mint/30">
        ...
        <RoleHero
          heroBadges={
            <DiagnosticHeroBadges
              label={content.heroBadgeLabel ?? DEFAULT_DIAGNOSTIC_LANDING_PAGE.heroBadgeLabel!}
            />
          }
          ...
          primaryCta={{
            label: content.heroPrimaryCtaLabel ?? DEFAULT_DIAGNOSTIC_LANDING_PAGE.heroPrimaryCtaLabel!,
      ...
      <WhyRellia
        sectionTitle={content.readinessTitle ?? DEFAULT_DIAGNOSTIC_LANDING_PAGE.readinessTitle!}
        sectionDescription={
          ...
          <div className="mx-auto flex w-full max-w-[1200px] flex-col gap-12 lg:flex-row lg:items-start">
            <div className="flex-1">
              <h2 className="mb-6 font-host-grotesk text-2xl font-semibold leading-tight text-black md:text-[32px]">
                {content.infographicTitle ?? DEFAULT_DIAGNOSTIC_LANDING_PAGE.infographicTitle}
              </h2>
      ...
      <MembershipPathTimeline
        headingTitle={content.timelineTitle ?? DEFAULT_DIAGNOSTIC_LANDING_PAGE.timelineTitle!}
        subheading={content.timelineSubheading ?? DEFAULT_DIAGNOSTIC_LANDING_PAGE.timelineSubheading}
      ...
      <div className="bg-white">
        <RelliaCta
          ...
          title={content.ctaTitle ?? DEFAULT_DIAGNOSTIC_LANDING_PAGE.ctaTitle!}
          body={content.ctaBody ?? DEFAULT_DIAGNOSTIC_LANDING_PAGE.ctaBody}
          primary={{
            label: content.ctaPrimaryLabel ?? DEFAULT_DIAGNOSTIC_LANDING_PAGE.ctaPrimaryLabel!,
            to: content.ctaPrimaryHref ?? DEFAULT_DIAGNOSTIC_LANDING_PAGE.ctaPrimaryHref!,
          }}
          secondary={{
            label: content.ctaSecondaryLabel ?? DEFAULT_DIAGNOSTIC_LANDING_PAGE.ctaSecondaryLabel!,
