...
const AdminNetworkProfileDetailPage = () => {
  ...
  const statusBanner = useMemo(() => {
    ...
    return {
      title: "No CMS draft yet",
      description: "This profile is in Airtable but has not been synced to Sanity. Create a CMS draft when the record is ready for review.",
      ...
      <div className="space-y-6">
        ...
        {profile && statusBanner ? (
          <>
                        ...
                        {section.tags && section.tags.length > 0 ? (
                          <div className="mt-3 flex flex-wrap gap-1.5">
                            {section.tags.map((tag) => (
                              <span
                                key={tag}
                                className="inline-flex rounded-full border border-border bg-muted/70 px-2.5 py-0.5 font-urbanist text-[11px] font-semibold text-foreground dark:border-rellia-mint/30 dark:bg-rellia-mint/15 dark:text-rellia-mint"
                              >
                                {tag}
                              </span>
                            ))}
            ...
            <Collapsible open={coverageOpen} onOpenChange={setCoverageOpen}>
              <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
                <CollapsibleTrigger className="flex w-full items-center justify-between gap-3 px-5 py-4 text-left">
                  <h2 className="font-host-grotesk text-base font-bold text-foreground md:text-lg">
                    Completion status
                  </h2>
                  <ChevronDown
            ...
            <div className={cn("rounded-2xl border p-5 md:p-6", adminTipBoxSurfaceClass)}>
              <div className="flex items-start gap-4 text-left">
                <span className={cn("h-11 w-11 shrink-0", adminIconTileClass)}>
                ...
                <div className="min-w-0">
                  ...
                  <ul className="mt-3 list-disc space-y-2 pl-5 font-urbanist text-sm leading-relaxed text-muted-foreground">
                    <li>
                      Click <strong className="text-foreground">Create CMS draft</strong> on this page when the
                      Airtable record is ready (or set <em>Website status → Ready for review</em> when automation is
                      enabled).
                    </li>
                    <li>Open Sanity Studio and review the draft copy, images, and directory tags.</li>
                    <li>
                      Click <strong className="text-foreground">Publish</strong> in Studio to go live. Publishing only
                      happens in Studio — not from this dashboard.
                    </li>
                  </ul>
                </div>
              </div>
