...
const buildEmailHTML = (analysis: AIResult, body: DiagnosticBody): string => {
  const scoreRows = body.sections
    .map(
      (s) =>
        `<tr><td style="padding:4px 8px;font-size:13px;color:#5a6a6e;">${s.title}</td>` +
  ...
  return `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;">
    <div style="background:#0c3d49;padding:2rem;border-radius:16px 16px 0 0;">
      ...
      <h1 style="color:white;font-size:1.6rem;margin:0 0 4px;">${body.company}</h1>
      <p style="color:rgba(255,255,255,0.5);font-size:13px;margin:0;">${body.stage} · ${new Date().toLocaleDateString("en-CA")}</p>
    </div>
...
export const handleDiagnosticAnalysis: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    aiResult = JSON.parse(text.replace(/```json|```/g, "").trim()) as AIResult
  } catch (err) {
