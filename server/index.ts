...
export function createServer() {
  ...
  // Default Helmet CSP blocks Vite HMR (dev) and eval / inline GTM / embeds (prod).
  // frame-ancestors is set manually (same as vercel.json) — do not enable Helmet's default script-src.
  ...
  /** Public single event — no CSRF; used by `/events/:slug` detail pages. */
  app.get(
    "/api/cms/events/:slug",
    rateLimitJson(sanityPublishedRate, SANITY_PUBLISHED_MAX_PER_MIN),
    async (req, res) => {
  ...
  /** Public ICS calendar feed for single event. Used by Apple Calendar integration. */
  app.get(
  ...
  app.get(
    ...
    async (req, res) => {
      ...
      try {
        ...
        if (users.length > 0) {
          const { data: presenceRows, error: presenceError } = await adminClient
            .from("admin_presence")
  ...
  const adminPresenceRate = new Map<string, RateState>();
  ...
  const adminTeamNoteRate = new Map<string, RateState>();
  const ADMIN_TEAM_NOTE_MAX_PER_MIN = 40;
  const adminTeamNoteReactionRate = new Map<string, RateState>();
  ...
  const sanitizeTeamNoteBlocks = (raw: unknown): TeamNoteBlock[] => {
    if (!Array.isArray(raw)) return [];
    const blocks: TeamNoteBlock[] = [];
      ...
      if (row.type === "image" && typeof row.url === "string") {
        ...
        if (url.startsWith("https://") || url.startsWith("/")) {
          blocks.push({
            type: "image",
            url,
            alt: typeof row.alt === "string" ? row.alt.trim().slice(0, 120) : undefined,
          });
        }
      }
    }
  ...
  const resolveAdminBearer = (req: express.Request): string => {
    const authHeader = typeof req.headers.authorization === "string" ? req.headers.authorization : "";
    ...
    async (req, res) => {
      ...
      try {
        ...
        const { data: board, error: boardError } = await adminClient
          ...
          .select("blocks, published_by_id, published_by_name, published_at, updated_at")
          .eq("id", "default")
          .maybeSingle();
        ...
        const { data: reactions, error: reactionsError } = await adminClient
          ...
          .select("emoji, user_id, user_name, created_at")
          .eq("board_id", "default");
  ...
  app.put(
    ...
    async (req, res) => {
      ...
      const blocks = sanitizeTeamNoteBlocks((req.body as { blocks?: unknown })?.blocks);
      if (blocks.length === 0) {
      ...
      try {
        ...
        const { data: board, error: upsertError } = await adminClient
          .from("admin_team_board")
          .upsert(
            {
              id: "default",
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      const emoji = typeof (req.body as { emoji?: unknown })?.emoji === "string"
        ? (req.body as { emoji: string }).emoji.trim().slice(0, 8)
        : "";
      if (!emoji) {
  ...
  app.get("/api/social-html", async (req, res) => {
    try {
      const pathParam =
      ...
      const { renderSocialHtmlForPath } = await import("./socialHtmlHandler.js")
      const rendered = await renderSocialHtmlForPath(pathParam)
      if (!rendered) {
