 ...
 *
 * Restrictions (env):
 * - SANITY_ALLOWED_DATASETS — comma-separated names; if set, any other dataset is rejected (server returns 503).
 * - SANITY_ENFORCE_VERCEL_DATASET=true — on Vercel, bind dataset to VERCEL_ENV (`preview` → `preview`, `production` → `production`).
 *   Overrides SANITY_API_DATASET / VITE_SANITY_DATASET for this server only (stops preview deploys from querying prod dataset).
 */
...
/** Narrow shape for call sites that only need success or null */
